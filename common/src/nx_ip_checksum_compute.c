...
/*                                                                        */
/**************************************************************************/
USHORT _nx_ip_checksum_compute(NX_PACKET *packet_ptr, ULONG protocol, UINT data_length, ULONG *src_ip_addr, ULONG *dest_ip_addr) {
  ULONG checksum = 0;
  USHORT tmp;
  USHORT *short_ptr;
  ULONG *long_ptr;
...
USHORT _nx_ip_checksum_compute2(NX_PACKET *packet_ptr, ULONG protocol, UINT data_length, ULONG *src_ip_addr, ULONG *dest_ip_addr) {
  ULONG checksum = 0;
  USHORT tmp;
  USHORT *short_ptr;
#ifndef NX_DISABLE_PACKET_CHAIN
  ...
  /* Setup the pointer to the start of the packet.  */
  /*lint -e{927} -e{826} suppress cast of pointer to pointer, since it is necessary  */
  short_ptr = (SHORT *)packet_ptr->nx_packet_prepend_ptr;
  ...
  while (current_packet) {
    ...
    /* Set the start address in this packet. */
    /*lint -e{927} -e{826} suppress cast of pointer to pointer, since it is necessary  */
    short_ptr = (SHORT *)current_packet->nx_packet_prepend_ptr;

    /*lint -e{946} suppress pointer subtraction, since it is necessary. */
    if ((ALIGN_TYPE)short_ptr < end_ptr) {
      /* Loop to calculate the packet's checksum.  */
      /*lint -e{946} suppress pointer subtraction, since it is necessary. */
      while ((ALIGN_TYPE)short_ptr < end_ptr) {
        checksum += (*short_ptr);
        short_ptr++;
        data_length -= 2;
      }
