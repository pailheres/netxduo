...
VOID _nx_ipv4_packet_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr) {
  ...
  if ((packet_ptr->nx_packet_length <= (ip_header_length << 2)) || (ip_header_length < NX_IP_NORMAL_LENGTH)) {
    /* Packet is too small!  */
...
#endif /* defined(NX_DISABLE_IP_RX_CHECKSUM) || defined(NX_ENABLE_INTERFACE_CAPABILITY) */
  {
    ...
    checksum = _nx_ip_checksum_compute2(packet_ptr, NX_IP_VERSION_V4,
  ...
  if (if_ptr->nx_interface_address_mapping_needed == NX_TRUE) {
    if (((ip_header_ptr->nx_ip_header_source_ip & ~(if_ptr->nx_interface_ip_network_mask)) == ~(if_ptr->nx_interface_ip_network_mask)) || (((ip_header_ptr->nx_ip_header_source_ip & ~(if_ptr->nx_interface_ip_network_mask)) == 0) && (ip_header_ptr->nx_ip_header_source_ip != 0)) || ((ip_header_ptr->nx_ip_header_source_ip & NX_IP_CLASS_D_MASK) == NX_IP_CLASS_D_TYPE)) {
#ifndef NX_DISABLE_IP_INFO
  ...
  if ((ip_header_ptr->nx_ip_header_destination_ip == if_ptr->nx_interface_ip_address) ||
      ...
      (((ip_header_ptr->nx_ip_header_destination_ip & if_ptr->nx_interface_ip_network_mask) == if_ptr->nx_interface_ip_network) && ((ip_header_ptr->nx_ip_header_destination_ip & ~(if_ptr->nx_interface_ip_network_mask)) == ~(if_ptr->nx_interface_ip_network_mask))) ||

      /* Check for limited broadcast.  */
      (ip_header_ptr->nx_ip_header_destination_ip == NX_IP_LIMITED_BROADCAST) ||

      /* Check for loopback address.  */
      ((ip_header_ptr->nx_ip_header_destination_ip >= NX_IP_LOOPBACK_FIRST) && (ip_header_ptr->nx_ip_header_destination_ip <= NX_IP_LOOPBACK_LAST)) ||
    ...
    if (ip_header_ptr->nx_ip_header_word_1 & NX_IP_FRAGMENT_MASK) {
      ...
      if (ip_ptr->nx_ip_fragment_assembly && (packet_ptr->nx_packet_pool_owner->nx_packet_pool_available >= packet_ptr->nx_packet_pool_owner->nx_packet_pool_low_watermark))
#else
      if (ip_ptr->nx_ip_fragment_assembly)
#endif
      {
        ...
        if ((ip_header_ptr->nx_ip_header_word_1 & NX_IP_MORE_FRAGMENT) && (((pkt_length - (ULONG)sizeof(NX_IPV4_HEADER)) & 0x7) != 0)) {
